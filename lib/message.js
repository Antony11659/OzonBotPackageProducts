export const mapMessages = {
    mainPage: {
      startMessage: "Привет выбери магазин: ",
      packMessage: "📦 Подготовь следуещее количество флаконов: \n",
      startSticking: "🏷️ Начать Клейть",
      amountOrders: "📦 Количество заказов равно: ", 
      threeMl: "Количество <i>трешек</i>: ",
      fiveMl: "Количество <i>пятишек</i>: ",
      tenMl: "Количество <i>десяток</i>: ",
      twentyMl: "Количество <i>двацаток</i>: ",
      thirtyMl: "Количество <i>трицаток</i>: ",
      fiftyMl: "Количество <i>полтиников</i>: "
    },
    sticking: {
        threeMl: "<i>трешек</i>: ",
        fiveMl: "<i>пятишек</i>: ",
        tenMl: "<i>десяток</i>: ",
        twentyMl: "<i>двацаток</i>: ",
        thirtyMl: "<i>трицаток</i>: ",
        fiftyMl: "<i>полтиников</i>: ",
        Next: "Следующее",
        Previous: "Предидущие",
        finishMessage: "🎉 Поклейка закончена!"
    },
    packaging: {
        Next: "Следующее",
        startPackaging: "Начать Упаковку",
        finishMessage: "🎉 Упаковкa закончена!"
    }
}