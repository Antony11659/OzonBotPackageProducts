 ...
 export const parseName = (str) => {
  ...
  const groupItems = (items) => {
    ...
    items.forEach((el) => {
        ...
        const increment = el.quantity;
        if(groupedItems[key]) {
            groupedItems[key].quantity += increment;
        } else {
            groupedItems[key] = {...el};
        }
    })
    ...
    try {
      ...
      for (const posting of result.postings) {
        for (const product of posting.products) {
          ...
          const productInfo = productList[sku];
          
          if (!productInfo) {
            const item = {
              ozonName: product.offer_id,
              name: `Новый аромат: ${product.offer_id}` ,
              volume: 0,
            ...
            const item = { ...productInfo, quantity: product.quantity };
            allItems.push(item);
        }
      }
  ...
  getStickingMessage (orders, page, amountPages) {
    ...
    const orderInfo = orders.map((order) => {
      ...
      const manyBottlesMessageLines = [
        "",
        `🌸 Название: <u><code>${perfumeName}</code></u>`,
        "",
        ...bottles,
        "",
