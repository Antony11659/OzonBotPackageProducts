...
const updateOrderVolume = (order, updatedOrder) => {
  ...
  if (order.volume === 3) {
    updatedOrder.threeMl += 1
  }
  if (order.volume === 5) {
    updatedOrder.fiveMl += 1
  }
  if (order.volume === 10) {
    updatedOrder.tenMl += 1
  }
  if (order.volume === 20) {
    updatedOrder.twentyMl += 1
  }
  if (order.volume === 30) {
    updatedOrder.thirtyMl += 1
  }
  if (order.volume === 50) {
    updatedOrder.fiftyMl += 1
  }
...
export const generateMessage = {
  ...
  getStickingMessage (orders, page, amountPages) {
    ...
    const messageLines = [`📃 Страница ${page + 1} из ${amountPages - 1}`,   '———————————————————', ...orderInfo]
    return messageLines.join("\n");
  },

  getPackageMessage (orders, currentPage, amountOfPages) {
    let message = `🧾 Страница ${currentPage} из ${amountOfPages}`
    const productItems = orders.map((order) => {
      let products = `♦️ ${order.postingNumber}: \n`
      for (const product of order.products) {
        const productLine = `${product.name} ${product.volume} ml\n`
...
export const packageOrders = async () => {
  const allItems = [];
  let cursor = "";
